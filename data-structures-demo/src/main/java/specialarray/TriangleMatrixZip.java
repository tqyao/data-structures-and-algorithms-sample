 ...
 */
public class TriangleMatrixZip {
...
//        return i < j ? this.arr1D[this.arr1D.length - 1] : this.arr1D[(i + 1) * i / 2 + j];
    ...
    private void print1DArray() {
        System.out.println(Arrays.toString(this.arr1D));
    }

    public static void main(String[] args) {
        int[][] triArray = downTriMatrixInit(5);
        print2DArray(triArray);

        TriangleMatrixZip arrayZip = new TriangleMatrixZip(triArray);
    ...
    private static void jugEqualForMatrixReflect(int[][] triArray, TriangleMatrixZip arrayZip) {
     ...
     */
    private static int getNSum(int n) {
        return (n + 1) * n / 2;
    }
     ...
     */
    private static int[][] downTriMatrixInit(int n) {
